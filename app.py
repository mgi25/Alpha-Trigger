...
def label_breakouts(df, tp=0.002, sl=0.001, lookahead: int = 3):
    ...
    for i in range(len(df) - lookahead):
        ...
        up_hit_idx = np.argmax(future_highs >= target_up) if np.any(future_highs >= target_up) else lookahead
        down_hit_idx = np.argmax(future_lows <= target_down) if np.any(future_lows <= target_down) else lookahead
...
def prepare_features(df):
    features = df[
        ["close", "volume", "atr", "atr_change", "atr_vs_avg",
         "vwap_dist_pct", "rsi", "macd_hist", "time_bucket"]
    ]
...
def main():
    ...
    if prob > 0.65:
        print("✅ High confidence trade signal 🔥")
    else:
        print("⚠️ Not worth it bro, sit this one out.")
